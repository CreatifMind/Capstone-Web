...
import { requireActiveDevelopmentWorkspace, type Role } from "@/lib/admin";
...
export async function modelReviewContext(allowedRoles?: readonly Role[]) {
  let context: Awaited<ReturnType<typeof requireActiveDevelopmentWorkspace>>;
  try { context = await requireActiveDevelopmentWorkspace(); } catch { return { response: failure("Authentication is not configured.", 503) }; }
